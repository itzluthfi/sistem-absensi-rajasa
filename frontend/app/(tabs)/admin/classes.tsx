...
export default function ClassesAdminScreen() {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter(
      (item) =>
        ...
        (item.major?.major_name || "").toLowerCase().includes(term) ||
        (item.homeroom_teacher?.full_name || "").toLowerCase().includes(term)
    );
  }, [records, query]);

  const handleExportClick = async () => {
    try {
      const data = await importExportApi.export("classes");
      ...
      link.href = url;
      link.setAttribute("download", `ekspor_kelas_${Date.now()}.xlsx`);
      document.body.appendChild(link);
      link.click();
      link.parentNode?.removeChild(link);
      window.URL.revokeObjectURL(url);
      toast.success("Ekspor data kelas berhasil diunduh.");
    } catch (error) {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const data = await importExportApi.template("classes");
      const blob = new Blob([data], {
        type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
      });
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", `templat_impor_kelas.xlsx`);
      document.body.appendChild(link);
      link.click();
      link.parentNode?.removeChild(link);
      window.URL.revokeObjectURL(url);
  ...
  const openEdit = (item: ClassRecord) => {
    setForm({
      id: String(item.id),
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (modalMode === "create") {
        ...
        await classesApi.update(Number(form.id), payload);
  ...
  return (
    <View style={[styles.container, { backgroundColor: "#F9FAFB" }]}>
      ...
      <View style={styles.headerTitleContainer}>
        <Text style={styles.headerTitle}>Master Data Kelas</Text>
        <Text style={styles.headerSubtitle}>Kelola rombongan belajar kelas, jurusan, dan wali kelas terpilih</Text>
      </View>
            ...
            <View style={{ borderRadius: 8, overflow: "hidden", borderWidth: 1, borderColor: "#E2E8F0" }}>
              ...
              {[1, 2, 3, 4, 5].map((i) => (
                <View key={i} style={styles.tableRow}>
                  ...
                  <View style={{ flex: 2 }}><Skeleton width={100} height={14} borderRadius={4} /></View>
                  <View style={{ flex: 2 }}><Skeleton width={120} height={14} borderRadius={4} /></View>
          ...
          ListEmptyComponent={
            <View style={styles.emptyState}>
              {isLoading ? (
                ...
                <>
                  <Ionicons name="business-outline" size={48} color="#1E3A8A" />
                  <Text style={styles.emptyTitle}>Data kelas belum tersedia</Text>
              ...
              <View style={styles.card}>
                ...
                <View style={styles.cardBody}>
                  ...
                  <Text style={styles.cardSubtitle}>
                    {[item.academic_year, item.major?.major_name, item.homeroom_teacher?.full_name]
                      .filter(Boolean)
                      .join(" | ") || "Kelas"}
                  </Text>
            ...
            <ScrollView style={styles.modalBody}>
              ...
              <View style={styles.inputGroup}>
                ...
                <TextInput
                  ...
                  keyboardType="numeric"
                  placeholder="ID Guru wali kelas"
                  placeholderTextColor="#9CA3AF"
                />
              </View>
            </ScrollView>
