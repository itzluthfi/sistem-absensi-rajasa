...
import { ActivityIndicator, Alert, ScrollView, StyleSheet, Text, TouchableOpacity, View, Image, useWindowDimensions } from 'react-native';
...
export default function ReportsScreen() {
  ...
  const canExport = hasRole(['super_admin', 'admin', 'kepala_sekolah', 'guru', 'wali_kelas']);
  ...
  const exportFormats: Array<{ key: ExportFormat; label: string; icon: IconName }> = [
    { key: 'pdf', label: 'PDF', icon: 'document-text-outline' },
    { key: 'csv', label: 'Excel/CSV', icon: 'grid-outline' },
  ];

  const preview = useMemo(() => {
    const days = Math.max(1, Math.ceil((new Date(dateRange.endDate).getTime() - new Date(dateRange.startDate).getTime()) / 86400000) + 1);
    return { days };
  }, [dateRange]);
  ...
  const applyPreset = (preset: 'week' | 'month' | 'year') => {
    ...
    setDateRange({ startDate: start.toISOString().split('T')[0], endDate: today.toISOString().split('T')[0] });
  };

  const handleExport = async () => {
    if (!canExport) {
      Alert.alert('Akses Ditolak', 'Akun ini tidak memiliki izin mengunduh laporan.');
      return;
    }
    setIsExporting(true);
    try {
      const params = { start_date: dateRange.startDate, end_date: dateRange.endDate };
      if (selectedFormat === 'pdf') await reportsApi.getAttendancePDF(params);
      else await reportsApi.getAttendanceCSV(params);
      Alert.alert('Berhasil', `Laporan ${selectedFormat.toUpperCase()} berhasil diminta dari server.`);
    } catch (error: any) {
      Alert.alert('Error', error.response?.data?.message || 'Gagal mengunduh laporan');
    } finally {
  ...
  return (
    <View style={[styles.container, { backgroundColor: 'transparent' }]}>
      ...
      <ScrollView style={{ flex: 1, backgroundColor: 'transparent' }} contentContainerStyle={[styles.content, { paddingBottom }]}>
        ...
        <Section title="Rentang Tanggal">
          ...
          <View style={styles.presetsRow}>
            <PresetButton label="Minggu Ini" onPress={() => applyPreset('week')} />
            <PresetButton label="Bulan Ini" onPress={() => applyPreset('month')} />
            <PresetButton label="Tahun Ini" onPress={() => applyPreset('year')} />
          </View>
        </Section>
        ...
        >
          {isExporting ? (
            ...
            <>
              <Ionicons name={selectedFormat === 'pdf' ? 'document-text-outline' : 'grid-outline'} size={20} color="#fff" />
              <Text style={styles.exportText}>{canExport ? 'Unduh Laporan' : 'Tidak Ada Akses Export'}</Text>
            </>
          )}
        </TouchableOpacity>
...
function OptionCard({ active, icon, label, activeColor, onPress }: { active: boolean; icon: IconName; label: string; activeColor: string; onPress: () => void }) {
  return (
    <TouchableOpacity style={[styles.optionCard, active && { borderColor: activeColor, backgroundColor: `${activeColor}12` }]} onPress={onPress}>
      <Ionicons name={icon} size={28} color={active ? activeColor : '#6B7280'} />
      <Text style={[styles.optionLabel, active && { color: activeColor }]}>{label}</Text>
    </TouchableOpacity>
  );
}
  ...
  return (
    <View style={styles.dateInput}>
      ...
      <View style={styles.dateValue}>
        <Text style={styles.dateText}>{new Date(value).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</Text>
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  optionCard: { flex: 1, minWidth: '45%', backgroundColor: '#F9FAFB', borderWidth: 2, borderColor: '#E5E7EB', borderRadius: 12, padding: 16, alignItems: 'center', gap: 8 },
  ...
  dateValue: { backgroundColor: '#F9FAFB', borderWidth: 1, borderColor: '#D1D5DB', borderRadius: 8, padding: 12 },
  dateText: { fontSize: 14, color: '#1F2937' },
  presetsRow: { flexDirection: 'row', gap: 8, marginTop: 12 },
  presetButton: { flex: 1, backgroundColor: '#EFF6FF', paddingVertical: 10, borderRadius: 8, alignItems: 'center' },
  presetText: { fontSize: 12, color: '#3B82F6', fontWeight: '700' },
  formatGrid: { flexDirection: 'row', gap: 12 },
  exportButton: { backgroundColor: '#10B981', borderRadius: 12, padding: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginBottom: 16, gap: 8 },
  exportButtonDisabled: { backgroundColor: '#9CA3AF' },
  exportText: { color: '#fff', fontSize: 16, fontWeight: '800' },
  previewCard: { backgroundColor: '#fff', borderRadius: 12, padding: 20, borderWidth: 1, borderColor: '#E5E7EB' },
  previewTitle: { fontSize: 18, fontWeight: '800', color: '#1F2937', marginBottom: 4 },
