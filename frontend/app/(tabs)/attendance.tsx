...
import { useAttendanceStore, type ScheduleRecord, type AttendanceRecord } from '../../store/attendanceStore';
...
export default function AttendanceScreen() {
  ...
  // Scan Mode
  const [scanMode, setScanMode] = useState<'none' | 'siswa_scan_guru' | 'guru_scan_siswa'>('none');
  ...
  const [studentOpsi, setStudentOpsi] = useState<'menu' | 'scan_guru' | 'tunjuk_qr'>('menu');
  ...
  const getGPSLocation = async () => {
    ...
    try {
      ...
      if (status !== 'granted') {
        Alert.alert('Izin Lokasi Diperlukan', 'Kami memerlukan akses lokasi Anda untuk memvalidasi presensi di dalam radius kelas.');
      ...
      Alert.alert('Gagal Mendeteksi Lokasi', 'Pastikan GPS HP Anda aktif dan coba lagi.');
      setLocationLoading(false);
      return false;
    }
  };
  ...
  const handleSiswaScanGuru = async ({ data }: { data: string }) => {
    ...
    try {
      ...
      Alert.alert('Format QR Tidak Valid', 'Pastikan Anda men-scan QR Code Sesi yang dipasang Guru di depan kelas.');
      setScanned(false);
    }
  };
  ...
  const handleGuruScanSiswa = async ({ data }: { data: string }) => {
    ...
    try {
      ...
      if (qrData.session_id !== activeSchedule?.active_session?.id) {
        Alert.alert('Sesi Berbeda', 'Siswa ini menampilkan QR untuk sesi pelajaran yang berbeda.');
        setScanned(false);
        return;
      }
      ...
      Alert.alert('Format QR Tidak Dikenal', 'Pastikan yang dipindai adalah QR Absen Siswa yang tertera di HP-nya.');
      setScanned(false);
    }
  };
  ...
  const handleCloseActiveSession = async () => {
    if (activeSchedule?.active_session) {
      Alert.alert('Tutup Presensi', 'Apakah Anda yakin ingin menutup sesi absensi kelas ini sekarang?', [
    ...
    if (!permission.granted) {
      return (
        <View style={styles.centeredContainer}>
          ...
          <Text style={styles.permissionText}>Aplikasi absensi memerlukan akses kamera untuk memindai QR Code.</Text>
          <TouchableOpacity style={styles.primaryButton} onPress={requestPermission}>
            <Text style={styles.buttonText}>Izinkan Akses Kamera</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.secondaryButton} onPress={() => setScanMode('none')}>
    ...
    return (
      <View style={styles.cameraScreen}>
        <CameraView
          ...
          facing="back"
          onBarcodeScanned={scanned ? undefined : (scanMode === 'siswa_scan_guru' ? handleSiswaScanGuru : handleGuruScanSiswa)}
        ...
        <View style={styles.cameraOverlay}>
          ...
          <Text style={styles.scannerInstruction}>
            {scanned ? 'Memproses presensi...' : 'Posisikan QR Code di dalam kotak'}
          </Text>
          <TouchableOpacity style={styles.cancelScanButton} onPress={() => setScanMode('none')}>
    ...
    <View style={[styles.container, { backgroundColor: 'transparent' }]}>
      ...
      <View style={styles.indicatorHeader}>
        ...
        <Text style={styles.indicatorText}>
          {isSiswa ? 'PRESENSI SISWA • RADJASA SECURE HIBRIDA' : 'KONTROL PRESENSI GURU • REALTIME FEED'}
        </Text>
      </View>
      ...
      {isLoadingSession ? (
          ...
          <ScrollView contentContainerStyle={[styles.scrollContent, { paddingBottom }]}>
            <View style={styles.sessionStatusCard}>
              <View style={styles.pulseIndicator}>
                ...
                <Text style={styles.pulseText}>SESI ABSENSI MATA PELAJARAN AKTIF</Text>
              </View>
            ...
            {studentOpsi === 'menu' ? (
              <View style={styles.opsiContainer}>
                ...
                >
                  <View style={[styles.opsiIconContainer, { backgroundColor: '#E6F4EA' }]}>
                    <Ionicons name="camera" size={28} color="#10B981" />
                  </View>
                  <View style={styles.opsiTexts}>
                    <Text style={styles.opsiCardTitle}>Opsi 1: Scan QR Guru (Rekomendasi)</Text>
                    <Text style={styles.opsiCardDesc}>Arahkan kamera HP Anda ke layar proyektor / HP Guru di depan kelas. Sistem memvalidasi radius GPS.</Text>
                  </View>
                ...
                >
                  <View style={[styles.opsiIconContainer, { backgroundColor: '#E0F2FE' }]}>
                    <Ionicons name="qr-code" size={28} color="#0284C7" />
                  </View>
                  <View style={styles.opsiTexts}>
                    <Text style={styles.opsiCardTitle}>Opsi 2: Tampilkan QR Absen Saya</Text>
                    <Text style={styles.opsiCardDesc}>Tunjukkan QR Code personal di HP Anda untuk di-scan oleh Guru menggunakan HP Guru di kelas.</Text>
                  </View>
                  <Ionicons name="chevron-forward" size={20} color="#9CA3AF" />
                </TouchableOpacity>
              </View>
              ...
              <View style={styles.qrDisplayCard}>
                <Text style={styles.qrTitle}>Tunjukkan QR Code Ini ke Guru</Text>
                <Text style={styles.qrSubtitle}>QR Code berisi token aman yang terikat ke jadwal aktif Anda hari ini.</Text>
                  ...
                  <Image
                    source={{
                      uri: `https://chart.googleapis.com/chart?cht=qr&chs=300x300&chl=${encodeURIComponent(
                        JSON.stringify({
                          student_id: (user?.student_info as any)?.id || user?.id,
                ...
                <Text style={styles.studentNisText}>NISN: {(user?.student_info as any)?.nisn || '00987654321'}</Text>
          ...
          <FlatList
            ...
            ListHeaderComponent={
              <View style={styles.teacherHeaderContainer}>
                ...
                <View style={styles.sessionStatusCard}>
                  <View style={styles.pulseIndicator}>
                    <View style={[styles.pulseCircle, { backgroundColor: '#3B82F6' }]} />
                    <Text style={[styles.pulseText, { color: '#3B82F6' }]}>SESI ABSENSI KELAS SEDANG BERLANGSUNG</Text>
                  </View>
                  <Text style={styles.sessionSubject}>{activeSchedule.subject?.subject_name}</Text>
                  <Text style={styles.sessionClass}>{activeSchedule.class?.class_name} • Jam: {activeSchedule.start_time.substring(0, 5)} - {activeSchedule.end_time.substring(0, 5)}</Text>
                ...
                <View style={styles.studentsListHeader}>
                  <Text style={styles.listTitle}>Siswa Hadir ({sessionDetail?.attendances?.length ?? 0} Orang)</Text>
                  <TouchableOpacity 
            ...
            renderItem={({ item }: { item: AttendanceRecord }) => (
              <View style={styles.attendanceListItem}>
                <View style={styles.studentProfileCircle}>
                  <Text style={styles.profileInitials}>{item.student?.full_name?.charAt(0).toUpperCase() || 'S'}</Text>
                </View>
                <View style={styles.studentListItemTexts}>
                  <Text style={styles.studentListName}>{item.student?.full_name || 'Siswa'}</Text>
                  <Text style={styles.studentListNis}>NIS: {item.student?.nis || ''} • Jam: {item.time?.substring(0, 5)}</Text>
                </View>
                <View style={[
        ...
        <View style={[styles.emptyContainer, { paddingBottom }]}>
          ...
          {!isSiswa && (
            ...
            >
              <Text style={styles.backHomeBtnText}>Ke Beranda & Buka Kelas</Text>
            </TouchableOpacity>
          )}
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  indicatorText: { color: '#fff', fontSize: 11, fontWeight: '800', letterSpacing: 0.5 },
  ...
  permissionTitle: { fontSize: 16, fontWeight: '800', color: '#1F2937', marginTop: 16, marginBottom: 4 },
  permissionText: { fontSize: 12, color: '#6B7280', textAlign: 'center', marginBottom: 20, paddingHorizontal: 16, lineHeight: 18 },
  primaryButton: { backgroundColor: '#2563EB', paddingHorizontal: 20, paddingVertical: 10, borderRadius: 10, marginBottom: 8 },
  ...
  cameraOverlay: { ...StyleSheet.absoluteFillObject, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.6)' },
  scannerTarget: { width: 240, height: 240, position: 'relative' },
  targetCorner: { position: 'absolute', width: 36, height: 36 },
  cTopLeft: { top: 0, left: 0, borderTopWidth: 4, borderLeftWidth: 4, borderColor: '#3B82F6' },
  cTopRight: { top: 0, right: 0, borderTopWidth: 4, borderRightWidth: 4, borderColor: '#3B82F6' },
  cBottomLeft: { bottom: 0, left: 0, borderBottomWidth: 4, borderLeftWidth: 4, borderColor: '#3B82F6' },
  cBottomRight: { bottom: 0, right: 0, borderBottomWidth: 4, borderRightWidth: 4, borderColor: '#3B82F6' },
  scannerInstruction: { color: '#fff', fontSize: 13, fontWeight: '800', backgroundColor: 'rgba(59,130,246,0.9)', paddingHorizontal: 16, paddingVertical: 8, borderRadius: 16, marginTop: 24, overflow: 'hidden' },
  cancelScanButton: { flexDirection: 'row', alignItems: 'center', gap: 6, position: 'absolute', bottom: 48, backgroundColor: 'rgba(0,0,0,0.7)', paddingVertical: 10, paddingHorizontal: 16, borderRadius: 10 },
  ...
  modalContent: { backgroundColor: '#fff', borderRadius: 24, padding: 32, alignItems: 'center', marginHorizontal: 32, shadowColor: '#000', shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.1, shadowRadius: 16, elevation: 6 },
